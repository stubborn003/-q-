...
class MapGame:
    def __init__(self, game_map):
        ...
        # Q学习参数
        self.q_table = np.zeros((self.rows, self.cols, 4))  # 4个方向的动作（上、下、左、右）
    ...
    def move_robot(self, start, end):
        ...
        for step in range(steps):
            ...
            self.robot_pos = (int(round(x)), int(round(y)))
    ...
    def get_action(self):
        ...
        if random.uniform(0, 1) < self.epsilon:
            return random.choice([0, 1, 2, 3])  # 随机选择动作
        else:
            # 选择Q值最高的动作
            return np.argmax(self.q_table[self.robot_pos[0], self.robot_pos[1]])

    def update_q_value(self, action, reward, next_state):
        # 更新Q值
        current_q = self.q_table[self.robot_pos[0], self.robot_pos[1], action]
        max_future_q = np.max(self.q_table[next_state[0], next_state[1]])
        new_q = current_q + self.learning_rate * (reward + self.discount_factor * max_future_q - current_q)
        self.q_table[self.robot_pos[0], self.robot_pos[1], action] = new_q
    ...
    def draw_robot(self, x, y):
        # 绘制机器人
        pygame.draw.rect(self.screen, BLUE, (y * self.cell_size, x * self.cell_size, self.cell_size, self.cell_size))
    ...
    def draw_map(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                # 根据地图内容选择颜色
                color = GREEN if self.map[i][j] == '#' else RED if self.map[i][j] == 'R' else BLACK
...
def load_map_from_file(filename):
    with open(filename, 'r') as file:
        return [list(line.strip()) for line in file.readlines()]  # 将每一行转换为列表


# 创建地图
game_map = load_map_from_file('target_map.txt')

# 创建并运行游戏
game = MapGame(game_map)
game.run()
